...
{
    public class AddUserToProjectCommandHandler : ICommandHandler<AddUserToProjectCommand>
    {
        private readonly ProjectWriteContext _projectWriteContext;

        public AddUserToProjectCommandHandler(ProjectWriteContext projectWriteContext)
        {
            _projectWriteContext = projectWriteContext;
        }

        public async Task HandleAsync(AddUserToProjectCommand command)
        {
            ...
            if (isProjectPresent)
            {
                ...
                var canChangeUsers = project.ProjectPersons.Any(pp =>
                    ...
                    pp.RemovedAt == null);
                var isInProject = project.ProjectPersons.Any(p => p.UserId == command.UserId &&
                                                                  p.RemovedAt == null);
                if (isInProject && canChangeUsers)
                {
                    var projectPerson = project.ProjectPersons.FirstOrDefault(pp => pp.UserId == command.UserId &&
                                                                                    pp.RemovedAt == null);
                    projectPerson.ChangePermission(command.PermissionType);
                }
                else
                {
                    project.ProjectPersons.Add(new ProjectPerson(command.UserId, command.PermissionType));
                }
